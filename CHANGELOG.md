...
### Added
- **Scroll animations for persona landing pages** — Added scroll-triggered entrance animations to all persona section components. Sections fade up or scale into view as the user scrolls, with staggered card reveals (150ms intervals) for grid/timeline layouts. Created `ScrollStagger` client component for stagger animation support. Uses existing CSS keyframe infrastructure (`scrollFadeUp`, `scrollScale`, `scroll-stagger`) and `ScrollAnimation` wrapper. Respects `prefers-reduced-motion`. ([ScrollStagger.tsx](apps/web/components/ScrollStagger.tsx), [OutputPreviewSection.tsx](apps/web/components/landing/sections/persona/OutputPreviewSection.tsx), [PainGapSection.tsx](apps/web/components/landing/sections/persona/PainGapSection.tsx), [DeliverableShowcaseSection.tsx](apps/web/components/landing/sections/persona/DeliverableShowcaseSection.tsx), [HowItWorksSection.tsx](apps/web/components/landing/sections/persona/HowItWorksSection.tsx), [PersonaTestimonialSection.tsx](apps/web/components/landing/sections/persona/PersonaTestimonialSection.tsx), [PersonaCtaSection.tsx](apps/web/components/landing/sections/persona/PersonaCtaSection.tsx))
- **Persona landing pages** — Created `/for-consultants` and `/for-product-leaders` landing pages as SEO-indexable Next.js server components. Each page has 7 sections targeting reptilian (Lionel mascot), limbic (loss aversion copy), and neocortex (proof/testimonials) brain layers. Consultant page uses timeline pain-gap variant with Wouter Chompff testimonial; product leader page uses grid variant with Sofiya Hodovanska testimonial. Includes `generateMetadata`, JSON-LD structured data, and domain routing via middleware. 7 reusable persona section components: `PersonaHeroSection`, `OutputPreviewSection`, `PainGapSection`, `DeliverableShowcaseSection`, `HowItWorksSection`, `PersonaTestimonialSection`, `PersonaCtaSection`. `OutputPreviewSection` renders a light-themed card on the dark background — follow-up email for consultants, product spec for product leaders — with realistic content that shows the exact output the persona will get. ([for-consultants/page.tsx](apps/web/app/[locale]/for-consultants/page.tsx), [for-product-leaders/page.tsx](apps/web/app/[locale]/for-product-leaders/page.tsx), [persona/](apps/web/components/landing/sections/persona/), [middleware.ts](apps/web/middleware.ts), [sitemap.ts](apps/web/app/sitemap.ts), [en.json](apps/web/messages/en.json))
- **Meta Pixel** — Added Meta/Facebook conversion tracking pixel (ID `1980144946219194`) to root layout with `PageView` tracking on all pages and `CompleteRegistration` event on both email and Google sign-up flows ([layout.tsx](apps/web/app/layout.tsx), [SignupForm.tsx](apps/web/components/SignupForm.tsx))
- **LinkedIn Insight Tag** — Added LinkedIn conversion tracking pixel (partner ID `8803626`) to root layout for site-wide retargeting and conversion attribution ([layout.tsx](apps/web/app/layout.tsx))
- **Email notification preferences UI** — Added email notification toggle to Settings → Preferences page with shadcn Switch component. Loads current preference from user profile on mount and persists via `PUT /user/email-notifications`. Includes optimistic update with rollback on error. Translations added for all 5 locales ([page.tsx](apps/web/app/[locale]/(authenticated)/settings/preferences/page.tsx), all locale JSON files)
- **Email notifications enabled by default** — New users are created with `emailNotifications.enabled: true`. Existing users without the field are treated as opted-in (opt-out model). UI defaults to on when field is missing ([user.repository.ts](apps/api/src/firebase/repositories/user.repository.ts), [email.service.ts](apps/api/src/email/email.service.ts))
- **Recent share email autocomplete** — Email addresses used for sharing conversations are now remembered per user (up to 20) and shown as autocomplete suggestions when sharing again. Supports keyboard navigation (arrow keys, Enter, Escape) and filters as you type. Stored on the user's Firestore document and fetched via `GET /user/recent-share-emails`. ([ShareModal.tsx](apps/web/components/ShareModal.tsx), [user-preferences.ts](apps/web/lib/user-preferences.ts), [user.controller.ts](apps/api/src/user/user.controller.ts), [transcription.service.ts](apps/api/src/transcription/transcription.service.ts), [types.ts](packages/shared/src/types.ts))
...
- **ESLint tsconfig for test files** — Added `tsconfig.eslint.json` that includes `src/**/*` and `test/**/*` so spec files are resolved by the TypeScript project service during linting, eliminating 56 parsing errors. ESLint config updated to use `project: './tsconfig.eslint.json'` instead of `projectService: true` ([tsconfig.eslint.json](apps/api/tsconfig.eslint.json), [eslint.config.mjs](apps/api/eslint.config.mjs))

### Changed
- **MBB-style redesign across all AI Asset templates** — Applied McKinsey/BCG design philosophy (Structure over prose, Format as signal, Scannability in 30 seconds) to all output templates. Renderer changes: Meeting Minutes discussion points now use chevron bullets with bold-before-colon labels; 1:1 Feedback sections upgraded to `variant="chevron"` + `boldBeforeColon`; Coaching Notes InsightCard gets category-based left-border colors (green=strength, amber=growth, purple=pattern) and priority badges on action items; Competitive Intel CompetitorCard gets threatLevel badge, italic positioning, and "Our edge:" differentiator callout; Objection Handler response renders as 3-step Acknowledge/Reframe/Bridge numbered list when array; Investor Update gets `oneLiner` purple pull-quote and `topAsk` amber highlight card; Recommendations Memo gets `topLine` purple pull-quote and priority-colored left-border rows on recommendations; All-Hands Talking Points gets `keyAnnouncements` section and handles `string | string[]` for opening/closing remarks; Vision Document risks grouped by level (high/medium/low) in color-coded cards; Press Release lead upgraded to dark pull-quote, quotes rendered as left-border inline cards. Prompt changes: CoachingNotes adds `category` (strength/growth/pattern) and action item `priority` (immediate/this-week/ongoing); InvestorUpdate adds `oneLiner` and `topAsk` fields; RecommendationsMemo adds `topLine` verdict field; CompetitiveIntel adds `threatLevel` and `differentiator` per competitor; ObjectionHandler response restructured as 3-step array; AllHandsTalking Points adds `keyAnnouncements` array. All new fields are additive/optional for backwards compatibility. ([analysis-templates.ts](apps/api/src/transcription/analysis-templates.ts), [types.ts](packages/shared/src/types.ts), multiple template TSX files)
- **PRD template — consultant-style output and visual redesign** — Rewrote PRD AI Asset prompt and schema for brevity and scannability. Problem statement is now an array of punchy diagnosis bullets (pain + quantified impact) instead of a prose paragraph. Removed User Stories section. Removed `rationale` field from requirements. Success Metrics capped at 2–3 KPIs. Renderer rebuilt: problem uses chevron bullets with bold-before-colon; Goals and Non-Goals shown side-by-side in a two-column layout; Requirements rendered as color-coded left-border rows (red/blue/amber by priority, flat sorted) replacing plain numbered list with sub-headings; Success Metrics rendered as metric cards in a 2-col grid with cyan accent; Won't Have requirements remain collapsible. Status badge color-coded by stage (draft/review/approved). ([analysis-templates.ts](apps/api/src/transcription/analysis-templates.ts), [PRDTemplate.tsx](apps/web/components/outputTemplates/PRDTemplate.tsx), [types.ts](packages/shared/src/types.ts))
- **Recording UI — focused active recording view** — Replaced the ambiguous recording state (which looked identical to the mic-test idle state) with a dedicated full-screen recording view. During active recording or pause: (1) X cancel button in top-right, (2) pulsing red dot + `Recording...` / `Paused` label, (3) giant `4.5rem` mono timer as dominant element, (4) tall 16-bar frequency visualization, (5) file size / remaining time info, (6) `Finish` and `Pause`/`Resume` as stacked full-width buttons. Paused state uses yellow indicator and dims the timer. ([SimpleAudioRecorder.tsx](apps/web/components/SimpleAudioRecorder.tsx))
- **Final CTA floating icons — constellation animation** — Replaced static floating icons around the video call image with an animated constellation of 8 asset icons (document, chart, email, checklist, lightbulb, microphone, target, pen). Icons expand outward from centre on scroll-into-view (staggered entrance), then settle into organic idle float with independent X/Y drift and subtle scale pulsing. Uses IntersectionObserver for scroll trigger and a two-layer DOM structure (outer: orbital positioning transition, inner: CSS float animation) to avoid transform conflicts. ([FloatingAssetIcons.tsx](apps/web/components/landing/shared/FloatingAssetIcons.tsx), [FinalCtaSection.tsx](apps/web/components/landing/sections/FinalCtaSection.tsx), [globals.css](apps/web/app/globals.css))
...
- **Custom domain for Google OAuth popup** — Changed Firebase `authDomain` from default `firebaseapp.com` to `app.neuralsummary.com` so the Google sign-in popup shows the custom domain. Added `/__/auth/*` rewrite in [next.config.ts](apps/web/next.config.ts) to proxy Firebase's auth handler, and excluded `__` paths from i18n middleware ([middleware.ts](apps/web/middleware.ts))

### Fixed
- **Email deliverability improvements** — Added `List-Unsubscribe` (RFC 8058), `List-Unsubscribe-Post`, and `Precedence: bulk` headers to all 6 email types to reduce spam classification. Added missing `replyTo` to 4 sendMail calls. Updated in-email unsubscribe link from `/settings` to `/settings/preferences` where the toggle now lives ([email.service.ts](apps/api/src/email/email.service.ts))
...
- **User profile menu migrated to shadcn DropdownMenu**: Replaced custom dropdown implementation (manual state, click-outside, positioning) with Radix UI primitives for better accessibility, keyboard navigation, and auto-positioning. Added theme switcher submenu (light/dark/system) with radio selection and icons
  - File: [UserProfileMenu.tsx](apps/web/components/UserProfileMenu.tsx)
- **Editorial summary layout with sticky Key Points sidebar**: Redesigned conversation page to a newspaper-style two-column layout. Key Points live in a sticky right-margin sidebar (always visible while scrolling). Drop-cap intro paragraph with Merriweather serif font, collapsible numbered "Deep Dives" accordion sections (all collapsed by default). Removed vertical Summary/Transcript tabs — transcript is now accessible via the three-dot menu. Mobile-optimized with shadcn Tabs toggle for Summary/Transcript, tighter spacing, and hidden editorial rule
  - Files: [SummaryRenderer.tsx](apps/web/components/SummaryRenderer.tsx), [ConversationClient.tsx](apps/web/app/[locale]/(authenticated)/conversation/[id]/ConversationClient.tsx), [ConversationSkeleton.tsx](apps/web/components/skeletons/ConversationSkeleton.tsx)
  - New dependencies: `@radix-ui/react-collapsible` via shadcn ([collapsible.tsx](apps/web/components/ui/collapsible.tsx))
