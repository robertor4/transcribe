...
### Added
- **Speaker Communication Assessment AI Asset template** — New per-speaker communication skills analysis. Scores each individual speaker on 6 dimensions (Clarity, Conciseness, Active Listening, Assertiveness, Questioning, Tone & Rapport) with evidence quotes and actionable tips. Features score rings, skill bars with progress indicators, and group dynamic summary. Uses GPT-5. ([analysis-templates.ts](apps/api/src/transcription/analysis-templates.ts), [SpeakerCommunicationAssessmentTemplate.tsx](apps/web/components/outputTemplates/SpeakerCommunicationAssessmentTemplate.tsx), [types.ts](packages/shared/src/types.ts))
- **Personality Profile AI Asset template** — Infers personality types per speaker using integrated MBTI/DISC/Big Five behavioral indicators. Features primary/secondary type labels, trait dimensions with confidence badges (high/medium/low), evidence-based assessments, communication preferences, working-with tips, team dynamics analysis, complementary strengths, and potential friction points. Includes responsible-use disclaimer. Uses GPT-5. ([analysis-templates.ts](apps/api/src/transcription/analysis-templates.ts), [PersonalityProfileTemplate.tsx](apps/web/components/outputTemplates/PersonalityProfileTemplate.tsx), [types.ts](packages/shared/src/types.ts))
