...
export class FirebaseService implements OnModuleInit {
  ...
  onModuleInit() {
    if (!admin.apps.length) {
      ...
      this.logger.log(`Firebase initialized with storage bucket: ${storageBucket}`);
    }
  ...
  async createTranscription(transcription: Omit<Transcription, 'id'>): Promise<string> {
    const docRef = await this.db.collection('transcriptions').add(transcription);
    return docRef.id;
  }
  ...
  async getTranscription(userId: string, id: string): Promise<Transcription | null> {
  ...
  async downloadFile(url: string): Promise<Buffer> {
    ...
    try {
      ...
      const bucketIndex = pathParts.findIndex(part => part === bucketName || part.includes('.firebasestorage.app') || part.includes('.appspot.com'));
