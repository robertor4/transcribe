...
// ─── Per-beat timing (synced to VO) ─────────────────────
// Beat start frames relative to montage start, and durations
// VO: 0:15–0:26 (team), 0:26–0:35 (sales), 0:36–0:45 (brainstorm), 0:46–0:55 (1:1)
const BEAT_TIMINGS = [
  { start: 0,   duration: 330 }, // 11s — team meeting
  { start: 330, duration: 270 }, // 9s  — sales call
  { start: 630, duration: 270 }, // 9s  — brainstorm (30-frame gap before)
  { start: 930, duration: 270 }, // 9s  — 1:1 (30-frame gap before)
];
...
/**
 * Scene 3: The Montage (0:15–0:55, 40s / 1200 frames)
 *
 * 4 beats synced to voiceover. Each beat:
 *  - Dark interstitial card with context text (Merriweather italic cyan)
 *  - 3 output card reveals in light-mode style
 */
export const TheMontage: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();

  // Determine which beat we're in based on per-beat timing
  let beatIndex = 0;
  for (let i = BEAT_TIMINGS.length - 1; i >= 0; i--) {
    if (frame >= BEAT_TIMINGS[i].start) {
      beatIndex = i;
      break;
    }
  }

  const timing = BEAT_TIMINGS[beatIndex];
  const beatFrame = frame - timing.start;
  const outputFramesPerCard = Math.floor((timing.duration - INTERSTITIAL_FRAMES) / 3);

  const beat = beats[beatIndex];
  const isInterstitial = beatFrame < INTERSTITIAL_FRAMES;
  const outputIndex = isInterstitial ? -1 : Math.min(Math.floor((beatFrame - INTERSTITIAL_FRAMES) / outputFramesPerCard), 2);
  const outputFrame = isInterstitial ? 0 : beatFrame - INTERSTITIAL_FRAMES - outputIndex * outputFramesPerCard;

  // Handle gaps between beats (dark background)
  const inGap = frame < timing.start || beatFrame >= timing.duration;

  // ─── Interstitial rendering ───────────────────────────
  const interstitialOpacity = isInterstitial && !inGap
    ? interpolate(
        beatFrame,
  ...
  const outputCardOpacity = !isInterstitial && !inGap
    ? interpolate(
        outputFrame,
        [0, 8, outputFramesPerCard - TRANSITION_FRAMES, outputFramesPerCard],
        [0, 1, 1, outputIndex < 2 ? 0 : 1],
        { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }
      )
    : 0;

  const outputCardScale = !isInterstitial && !inGap
  ...
  const currentOutput = !isInterstitial && !inGap && outputIndex >= 0 ? beat.outputs[outputIndex] : null;
