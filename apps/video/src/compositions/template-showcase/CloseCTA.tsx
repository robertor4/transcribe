...
export const ShowcaseCloseCTA: React.FC = () => {
  ...
  // Wave divider fades in (0–0.5s)
  const waveOpacity = interpolate(frame, [0, fps * 0.5], [0, 1], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });

  // Logo fades in (0.3–1s)
  const logoOpacity = interpolate(frame, [fps * 0.3, fps * 1.0], [0, 1], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
  const logoY = interpolate(frame, [fps * 0.3, fps * 1.0], [20, 0], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });

  // Headline fades in (1.2–2s)
  const headlineOpacity = interpolate(frame, [fps * 1.2, fps * 2.0], [0, 1], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
  const headlineY = interpolate(frame, [fps * 1.2, fps * 2.0], [25, 0], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });

  // CTA button springs in (2.5s)
  ...
  // URL fades in (3s)
  const urlOpacity = interpolate(frame, [fps * 3.0, fps * 3.5], [0, 1], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
