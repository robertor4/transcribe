...
/**
 * Scene 3: Template Selector Opens (0:14–0:22)
 *
...
export const TemplateSelector: React.FC = () => {
  ...
  // Phase 1: Conversation page (0–2s)
  const showModal = frame > fps * 1.5;
  const pageFade = interpolate(frame, [fps * 1.5, fps * 2.0], [1, 0.12], {
  ...
  const modalScale = showModal
    ? spring({ frame: frame - fps * 1.5, fps, config: { damping: 18 } })
    : 0;

  // Template list scroll (3–7s) — scroll position in px
  const scrollOffset = interpolate(frame, [fps * 3.0, fps * 7.0], [0, 650], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });

  // "Choose from 40 templates." overlay
  const overlayOpacity = interpolate(frame, [fps * 6.5, fps * 7.0], [0, 1], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });

  // Exit fade
  const exitFade = interpolate(frame, [fps * 7.5, fps * 8.0], [1, 0], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
              ...
              <div style={{ flex: 1, overflow: 'hidden', position: 'relative' }}>
                ...
                >
                  {categories.map((cat, catIdx) => {
                    ...
                    const catStart = fps * 2.0 + catIdx * 6;
