...
export function getApiUrl(): string {
  ...
  // In the browser, if the hostname isn't localhost, use the /api proxy
  // (e.g. accessing dev server via Cloudflare tunnel or LAN IP)
  if (typeof window !== 'undefined') {
    const h = window.location.hostname;
  ...
  // In development on localhost, use direct API URL
...
export function getWebSocketUrl(): string {
  ...
  if (typeof window !== 'undefined') {
    const h = window.location.hostname;
    if (isProduction() || (h !== 'localhost' && h !== '127.0.0.1')) {
      // Use current host for production, tunnels, or LAN access
